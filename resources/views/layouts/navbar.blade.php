<nav class="border-b border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="{{ route('home') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img src="{{ asset($setting->logo) }}" class="h-8" alt="{{ $setting->site_name }}" />
            @if (request()->routeIs('home'))
                <h1 class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white sr-only">
                    {{ $setting->site_name }}</h1>
            @else
                <span
                    class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white sr-only">{{ $setting->site_name }}</span>
            @endif
        </a>
        <div class="flex items-center space-x-2">
            @auth
                <div class="flex items-center">
                    ...
                    <span class="font-medium text-gray-700 leading-normal">
                        {{ auth()->user()->coins + 0 }}
                    </span>
                </div>
            @endauth
            <button type="button"
                class="inline-flex items-center justify-center p-2 w-10 h-10 text-sm text-gray-500 rounded-lg hover:bg-gray-100 focus:outline-none dark:text-gray-400 dark:hover:bg-gray-700"
                data-drawer-target="navbar-hamburger" data-drawer-show="navbar-hamburger"
        ...
        <div id="navbar-hamburger"
            class="absolute top-0 left-0 z-50 h-screen overflow-y-auto transition-transform -translate-x-full bg-white w-full dark:bg-gray-800"
            tabindex="-1" aria-labelledby="drawer-label">
            <div class="flex items-center justify-between p-4">
                <img src="{{ asset($setting->logo) }}" class="h-8" alt="{{ $setting->site_name }}" />
                <span id="drawer-label"
                    class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white sr-only">{{ $setting->site_name }}</span>
                <button type="button" data-drawer-hide="navbar-hamburger" aria-controls="navbar-hamburger"
            ...
            <div class="px-4">
                <ul
                    class="flex flex-col font-medium mt-4 rounded-lg dark:border-gray-700 list-none px-0 *:mb-0 space-y-2">
                    @foreach ($pages as $page)
                        <li>
                            <a href="{{ route('page', $page->slug) }}"
                                class="block py-2 px-3 text-gray-900 rounded-lg dark:bg-gray-800 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{{ $page->title }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
